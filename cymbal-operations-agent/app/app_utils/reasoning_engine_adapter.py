...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    def normalize_input(method, raw_input: dict | None) -> dict:
        kwargs = dict(raw_input or {})
        try:
        ...
        if "user_id" in params and "user_id" not in kwargs:
            kwargs["user_id"] = "default_user"

        if "message" in params and "message" not in kwargs:
            for alt in ("prompt", "input", "query", "text"):
                if alt in kwargs:
    ...
    async def stream_query(request: Request) -> responses.StreamingResponse:
        ...
        kwargs = normalize_input(method, body.get("input"))
